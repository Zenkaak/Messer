    ...
    try {
      const resp = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          text: message.text,
          headers: {
            "Message-ID": `<${Date.now()}.${Math.random().toString(36).slice(2, 10)}@gsmworld.co.ke>`,
            "X-Entity-Ref-ID": `gsm-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
            "Precedence": "transactional",
            "X-Auto-Response-Suppress": "All",
            "List-Unsubscribe": `<${unsubscribeUrl}>, <mailto:unsubscribe@gsmworld.co.ke?subject=unsubscribe>`,
