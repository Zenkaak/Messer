...
function infoTable(rows: Array<[string, string]>) {
  const trs = rows.map(([label, value]) => `<tr>
    <td style="padding:11px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:600;color:#64748b;white-space:nowrap;width:40%;">${label}</td>
    <td style="padding:11px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:700;color:#0f172a;text-align:right;">${value}</td>
  </tr>`).join("");
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;margin:20px 0;">
    <tbody style="display:table;width:100%;padding:0 20px;">${trs}</tbody>
  </table>`;
}
...
export function loginNotificationEmail(name: string | null, meta?: { ip?: string; device?: string }) {
  ...
  `;
  return {
    subject: "Security Alert: New Sign-In to Your GSM World Account",
    ...
    html: layout(`Security alert: new sign-in on your GSM World account at ${time}.`, "#dc2626", h, body),
  };
}
...
export function orderSubmittedEmail(params: {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#475569;">Our team will review and process your order. You will receive a follow-up email once your order status changes. Estimated processing time is <strong style="color:#0f172a;">1 – 24 hours</strong>.</p>
...
export function paymentConfirmedEmail(params: {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#475569;">Our team is now working on your order. You will receive another notification once it is completed, typically within <strong style="color:#0f172a;">1 – 24 hours</strong>.</p>
