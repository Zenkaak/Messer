...
// ─── WebAuthn in-memory state ──────────────────────────────────────────────────
interface WaChallengeEntry { challenge: string; origin: string; rpID: string; ts: number; }
const _waChallenges = new Map<"register" | "auth", WaChallengeEntry>();
const _waSessions = new Map<string, number>(); // token → expiry timestamp
const WA_CHALLENGE_TTL = 5 * 60 * 1000;
const WA_SESSION_TTL = 24 * 60 * 60 * 1000;

function _cleanupWaSessions() {
  const now = Date.now();
  for (const [t, exp] of _waSessions) if (now > exp) _waSessions.delete(t);
}
...
async function checkAdminAuth(req: import("express").Request, res: import("express").Response): Promise<boolean> {
  ...
  if (token) {
    _cleanupWaSessions();
    const exp = _waSessions.get(token);
...
router.post("/admin/login", async (req, res) => {
  ...
  try {
    const { blocked, retryAfterSec } = await checkRateLimit(`rl:admin_login:${ip}`, ADMIN_LOGIN_MAX, ADMIN_LOGIN_WINDOW_MS);
    if (blocked) {
    ...
    if (!(await checkAdminPassword(String(password)))) {
      await recordRateLimitAttempt(`rl:admin_login:${ip}`, ADMIN_LOGIN_WINDOW_MS);
      res.status(401).json({ error: "Invalid password" });
      return;
    }
    await clearRateLimit(`rl:admin_login:${ip}`);
    ...
    switch (type) {
      ...
      case "pending_manual":
        emailPayload = pendingManualPaymentEmail({
          ...
          total: SAMPLE_TOTAL,
          binanceId: "490759406",
          usdtAddress: "TGx7myR8nQeixT32ytm1z5agD4uHyYtRaB",
        });
        break;
      default:
...
router.get("/admin/webauthn/status", async (req, res) => {
  try {
    const cred = await getWebauthnCredential();
    res.json({ registered: Boolean(cred) });
  } catch (err) {
    req.log.error({ err }, "webauthn status error");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.post("/admin/webauthn/register-challenge", async (req, res) => {
  try {
    ...
    const options = await generateRegistrationOptions({
      ...
      rpID,
      userName: "admin",
      userDisplayName: "GSM World Admin",
...
router.delete("/admin/webauthn/credential", async (req, res) => {
  try {
    if (!(await checkAdminAuth(req, res))) return;
    await deleteWebauthnCredential();
    res.json({ success: true });
  } catch (err) {
    req.log.error({ err }, "webauthn delete credential error");
    res.status(500).json({ error: "Internal server error" });
  }
});

router.post("/admin/webauthn/auth-challenge", async (req, res) => {
  try {
...
router.post("/admin/webauthn/auth", async (req, res) => {
  try {
    ...
    _waSessions.set(token, Date.now() + WA_SESSION_TTL);
