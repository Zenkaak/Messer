...
import { sendEmail, appUrl, orderSubmittedEmail, pendingManualPaymentEmail, paymentConfirmedEmail } from "../lib/email";
import { z } from "zod";
...
const USDT_TRC20_ADDRESS = "TNgDQqmgQo5soUH8pGv6LgB69zCVCS7gq5";
...
router.post("/checkout", async (req, res) => {
  try {
    ...
    const { customerEmail, customerPhone, customerName, paymentMethod } = parsed.data;
    ...
    if (paymentMethod === "wallet") {
      ...
      await db.insert(paymentTransactionsTable).values({
        ...
        currency: "USD",
        status: "pending",
        rawResponse: { binanceId: BINANCE_ID, reference: `ORDER-${order.id}` } as Record<string, unknown>,
      });
      await db.delete(cartItemsTable).where(eq(cartItemsTable.sessionId, sessionId));
      customData = { method: "binance_pay", binanceId: BINANCE_ID, amount: total, reference: `ORDER-${order.id}` };
      responseStatus = "pending_payment_confirmation";
      // Send pending payment email
      sendEmail({
        to: customerEmail,
        ...pendingManualPaymentEmail({ orderId: order.id, customerName, paymentMethod: "binance_pay", total: String(total), binanceId: BINANCE_ID }),
      ...
      await db.insert(paymentTransactionsTable).values({
        ...
        currency: "USDT",
        status: "pending",
        rawResponse: { address: USDT_TRC20_ADDRESS, network: "TRC20", reference: `ORDER-${order.id}` } as Record<string, unknown>,
      });
      await db.delete(cartItemsTable).where(eq(cartItemsTable.sessionId, sessionId));
      customData = { method: "usdt_manual", address: USDT_TRC20_ADDRESS, network: "TRC20", amount: total, reference: `ORDER-${order.id}` };
      responseStatus = "pending_payment_confirmation";
      // Send pending payment email
      sendEmail({
        to: customerEmail,
        ...pendingManualPaymentEmail({ orderId: order.id, customerName, paymentMethod: "usdt_manual", total: String(total), usdtAddress: USDT_TRC20_ADDRESS }),
      }).catch((err) => logger.error({ err }, "Failed to send pending payment email"));
    } else {
    ...
    if (paymentMethod !== "binance_pay" && paymentMethod !== "usdt_manual") {
      ...
      sendEmail({
        to: customerEmail,
        ...orderSubmittedEmail({ orderId: order.id, customerName, items, total: String(total), paymentMethod }),
