...
import { Wallet, Plus, ChevronRight, Zap, Server, Star } from "lucide-react";
...
function extractCreditQty(name: string): number | null {
  const match = name.match(/\b(\d[\d,]*)\s*(credits?|cr\b)/i);
  if (match) {
...
function CreditCard({ product }: { product: { id: number; name: string; price: number; categoryName?: string | null; imageUrl: string; inStock: boolean } }) {
  const creditQty = extractCreditQty(product.name);
  const perCredit = creditQty && creditQty > 0 ? product.price / creditQty : null;
            ...
            {perCredit !== null ? (
              <>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[13px] font-black text-green-600">${perCredit < 0.01 ? perCredit.toFixed(4) : perCredit < 0.1 ? perCredit.toFixed(3) : perCredit.toFixed(2)}/credit</span>
                </div>
                <span className="text-[10px] text-gray-400">{creditQty?.toLocaleString()} credits · ${product.price.toFixed(2)} total</span>
              </>
            ) : (
              <div className="flex items-center gap-1.5">
                <span className="bg-orange-100 text-orange-600 text-[9px] font-black px-1.5 py-0.5 rounded-full">Server</span>
