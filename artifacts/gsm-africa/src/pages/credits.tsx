...
import { ShoppingCart, Check, Wallet, Plus, ChevronRight, Zap, Server, Star } from "lucide-react";
...
function CreditCard({ product }: { product: { id: number; name: string; price: number; categoryName?: string | null; imageUrl: string; inStock: boolean } }) {
  ...
  function handleBuy(e: React.MouseEvent) {
    ...
    addToCart.mutate({ data: { productId: product.id, quantity: 1 } }, {
      ...
      onError: () => { setAdding(false); toast({ variant: "destructive", title: "Could not add to cart" }); },
    });
  }
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-3.5 shadow-sm hover:shadow-md transition-all flex items-center gap-3">
        ...
        <button
          ...
          className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
            added ? "bg-green-600 text-white" : adding ? "bg-gray-100 text-gray-400" : "bg-blue-600 text-white hover:bg-blue-700"
          }`}
        >
          {adding ? (
