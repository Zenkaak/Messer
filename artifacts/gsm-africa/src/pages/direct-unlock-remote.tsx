        ...
        <div className="mt-3">
          {devices.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center"><Search size={21} className="mx-auto text-slate-400" /><p className="mt-3 text-sm font-bold text-slate-700">No matching services</p><p className="mt-1 text-xs text-slate-500">Try a different model name or clear the search.</p></div> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {devices.map((model) => {
              ...
              return <button type="button" key={model.name} onClick={() => onSelect(device)} className={`group flex min-h-[112px] flex-col rounded-xl border p-3 text-left transition-all ${isSelected ? "border-[#00bda9] bg-[#edfcf9] shadow-[0_8px_18px_rgba(0,189,169,0.12)]" : "border-slate-200 bg-white hover:-translate-y-0.5 hover:border-[#8edfd5] hover:shadow-[0_8px_18px_rgba(15,23,42,0.08)]"}`}><span className="flex items-start justify-between gap-2"><span className={`grid h-8 w-8 place-items-center rounded-lg ${isSelected ? "bg-[#00bda9] text-white" : "bg-slate-100 text-slate-500 group-hover:bg-[#e8fbf8] group-hover:text-[#008b99]"}`}><Smartphone size={15} /></span><ChevronRight size={14} className={isSelected ? "mt-1 text-[#008b99]" : "mt-1 text-slate-300 group-hover:text-[#008b99]"} /></span><span className="mt-2 min-h-[32px] text-[11px] font-black leading-4 text-slate-800">{model.name}</span><span className="mt-2 flex items-center justify-between gap-2 border-t border-slate-100 pt-2"><span className="flex items-center gap-1 text-[9px] font-semibold text-emerald-600"><CheckCircle2 size={11} /> Verified service</span><span className="shrink-0 rounded-full bg-[#e2faf5] px-2 py-1 font-mono text-[9px] font-black text-[#007c89]">{money(model.price)}</span></span></button>;
            })}
          </div>}
        </div>
