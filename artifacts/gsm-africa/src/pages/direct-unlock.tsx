...
import { Smartphone, ChevronRight, CheckCircle2, ArrowLeft, Cpu, Lock, AlertCircle, RefreshCw, Copy, Check } from "lucide-react";
...
export function DirectUnlockPage() {
  ...
  async function handleManualPay() {
    ...
    try {
      ...
      setOrderId(data.id!);
      setManualDone(true);
      toast({ title: "Order placed!", description: "Send payment and our team will verify within 24h." });
    } catch (err) {
      ...
      {step === "imei" && selectedBrand && selectedModel && (
        <div className="space-y-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-sm font-bold text-gray-900">Enter device details</p>
          </div>
          ...
          {!mpCheckoutId && !npPayment && (
            <div className="space-y-2">
              {([
                ...
                { id: "binance_pay" as PayMethod, label: "Binance Pay", icon: "🟡", sub: "Manual — Admin verifies within 24h", warn: false },
                { id: "usdt_manual" as PayMethod, label: "USDT TRC20 (Manual)", icon: "💲", sub: "Send to our address — Admin verifies within 24h", warn: false },
          ...
          {payMethod === "binance_pay" && !mpCheckoutId && !npPayment && (
            manualDone && orderId ? (
              <div className="bg-white border border-green-200 rounded-2xl p-5 space-y-3 text-center">
                ...
                <p className="text-sm text-gray-500">Our team verifies your payment within <strong>24 hours</strong>.</p>
              </div>
            ) : (
              <div className="space-y-3">
