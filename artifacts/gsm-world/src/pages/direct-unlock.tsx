...
const BRAND_LOGOS: Record<string, string> = {
  ...
  "iPhone / iCloud":       "https://cdn.simpleicons.org/apple/ffffff",
  ...
  "Xiaomi / Redmi / POCO": "https://cdn.simpleicons.org/xiaomi/ffffff",
  "Sony":                  "https://cdn.simpleicons.org/sony/ffffff",
  "OnePlus":               "https://cdn.simpleicons.org/oneplus/ffffff",
  "Oppo / Realme":         "https://cdn.simpleicons.org/oppo/ffffff",
  "Vivo":                  "https://cdn.simpleicons.org/vivo/ffffff",
  "Google Pixel":          "https://cdn.simpleicons.org/google/ffffff",
  "TCL / Alcatel":         "https://cdn.simpleicons.org/tcl/ffffff",
  "ZTE":                   "https://cdn.simpleicons.org/zte/ffffff",
};

const BRAND_COLORS: Record<string, string> = {
  "Samsung":               "#1428A0",
  "iPhone / iCloud":       "#1c1c1e",
  "iPad":                  "#3d3d5c",
  "Huawei":                "#CF0A2C",
  "Nokia":                 "#124191",
  "LG":                    "#A50034",
...
// ── Sidebar ──────────────────────────────────────────────────────────────────
function InfoSidebar({ walletBalance, selectedModel, isAuthenticated }: { walletBalance: number; selectedModel: { name: string; price: number } | null; isAuthenticated: boolean }) {
  return (
    <aside className="w-full md:w-72 lg:w-80 shrink-0 space-y-4">
      {/* Service stats card */}
      <div className="rounded-2xl overflow-hidden" style={{ background: "linear-gradient(135deg,#0a1628 0%,#0d1e38 100%)", border: "1px solid rgba(59,130,246,0.25)" }}>
        <div className="p-5">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg,#2563eb,#4f46e5)" }}>
              <Cpu size={20} className="text-white" />
            </div>
            <div>
              <p className="font-black text-base leading-none text-white">Direct Unlock</p>
              <p className="text-[11px] mt-0.5 font-medium" style={{ color: "#6ea4d4" }}>Professional Network Unlock</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 mb-4">
            {[
              ...
              <div key={s.label} className="rounded-xl px-3 py-2.5 text-center" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(59,130,246,0.12)" }}>
                <p className="text-white font-black text-lg leading-none">{s.value}</p>
                <p className="text-[10px] mt-0.5 font-medium" style={{ color: "#4a6a8a" }}>{s.label}</p>
              </div>
            ))}
          </div>
          {isAuthenticated && (
            <div className="flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(59,130,246,0.15)" }}>
              <Wallet size={14} style={{ color: "#6ea4d4" }} className="shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-medium" style={{ color: "#4a6a8a" }}>Wallet Balance</p>
                <p className="text-white font-black text-sm">${walletBalance.toFixed(2)}</p>
              </div>
              <Link href="/account/add-fund">
                <button className="text-[10px] font-bold px-2 py-1 rounded-lg transition-colors" style={{ background: "rgba(37,99,235,0.25)", color: "#93c5fd", border: "1px solid rgba(59,130,246,0.3)" }}>Top Up</button>
              </Link>
            </div>
          )}
        </div>
        ...
        {selectedModel && (
          <div className="px-4 py-3" style={{ borderTop: "1px solid rgba(59,130,246,0.12)", background: "rgba(255,255,255,0.03)" }}>
            <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: "#2d4a6a" }}>Current Order</p>
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-white truncate flex-1 pr-2">{selectedModel.name}</p>
              <p className="text-lg font-black shrink-0" style={{ color: "#34d399" }}>${selectedModel.price}</p>
            </div>
          </div>
        )}
      </div>
      ...
      {/* How it works */}
      <div className="rounded-2xl p-4" style={{ background: "#0d1b35", border: "1px solid rgba(59,130,246,0.15)" }}>
        <p className="text-xs font-black uppercase tracking-wider mb-3 flex items-center gap-2" style={{ color: "#6ea4d4" }}>
          <Info size={12} style={{ color: "#60a5fa" }} /> How It Works
        </p>
        <div className="space-y-3">
          {[
            ...
            <div key={s.n} className="flex items-start gap-3">
              <div className="w-6 h-6 rounded-lg flex items-center justify-center text-[11px] font-black shrink-0 mt-0.5" style={{ background: "rgba(37,99,235,0.2)", color: "#60a5fa", border: "1px solid rgba(59,130,246,0.25)" }}>{s.n}</div>
              <div>
                <p className="text-[12px] font-bold text-white">{s.title}</p>
                <p className="text-[11px] leading-snug" style={{ color: "#4a6a8a" }}>{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Trust */}
      <div className="rounded-2xl p-4 space-y-2.5" style={{ background: "#0d1b35", border: "1px solid rgba(59,130,246,0.15)" }}>
        <p className="text-xs font-black uppercase tracking-wider mb-1 flex items-center gap-2" style={{ color: "#6ea4d4" }}>
          <Shield size={12} className="text-emerald-500" /> Security & Trust
        </p>
        {[
          { icon: <Shield size={13} className="text-emerald-400" />, text: "256-bit SSL encryption on all requests" },
          { icon: <BadgeCheck size={13} className="text-blue-400" />, text: "GSMA-compliant carrier unlock process" },
          { icon: <Users size={13} className="text-purple-400" />, text: "15,000+ successful unlocks completed" },
          { icon: <Star size={13} className="text-yellow-400" />, text: "4.9/5 average customer rating" },
          { icon: <Zap size={13} className="text-orange-400" />, text: "Permanent unlock — no relocking ever" },
        ].map((t, i) => (
          <div key={i} className="flex items-start gap-2.5">
            <span className="mt-0.5 shrink-0">{t.icon}</span>
            <p className="text-[11px] leading-snug" style={{ color: "#6ea4d4" }}>{t.text}</p>
          </div>
        ))}
      </div>

      {/* Support */}
      <div className="rounded-2xl p-4" style={{ background: "linear-gradient(135deg,rgba(37,99,235,0.12),rgba(79,70,229,0.12))", border: "1px solid rgba(59,130,246,0.2)" }}>
        <p className="text-xs font-black text-white mb-1">Need Help?</p>
        <p className="text-[11px] mb-3" style={{ color: "#6ea4d4" }}>Our support team is available 24/7 for unlock assistance.</p>
        <a href="/chat" className="flex items-center justify-center gap-2 text-white text-[12px] font-bold py-2 px-4 rounded-xl transition-colors" style={{ background: "linear-gradient(135deg,#2563eb,#4f46e5)" }}>
...
// ── UnlockTool-style colored brand grid ───────────────────────────────────────
function TopBrandBar({ selectedBrand, onSelect }: {
  selectedBrand: typeof DEVICE_CATALOG[0] | null;
  onSelect: (brand: typeof DEVICE_CATALOG[0]) => void;
}) {
  return (
    <div style={{ background: "#06101e", borderBottom: "2px solid #0d1e38", padding: "6px 6px" }}>
      <div className="grid gap-1" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(82px, 1fr))" }}>
        {DEVICE_CATALOG.map(brand => {
          const isActive = selectedBrand?.brand === brand.brand;
          const color = BRAND_COLORS[brand.brand] ?? "#1a2a3a";
          const short = BRAND_SHORT[brand.brand] ?? brand.brand;
          ...
          return (
            <button
              ...
              style={{
                ...
                border: `2px solid ${isActive ? color : "rgba(255,255,255,0.04)"}`,
                boxShadow: isActive ? `0 0 18px ${color}55, inset 0 1px 0 rgba(255,255,255,0.15)` : "none",
                borderRadius: "8px",
                padding: "9px 4px 7px",
                display: "flex",
            ...
            >
              ...
              <div style={{ width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center" }}>
                {logoUrl
                  ? <img src={logoUrl} alt={brand.brand} style={{ width: 22, height: 22, objectFit: "contain", filter: isActive ? "none" : "opacity(0.6)" }} />
                  : <span style={{ fontSize: 18, lineHeight: 1 }}>{brand.icon}</span>
                }
              </div>
              <span style={{
                fontSize: 9,
...
export function DirectUnlockPage() {
  ...
  // Auth gate moved to payment step — dashboard is always visible
  ...
  // brand-bar handler
  function handleBrandSelect(brand: typeof DEVICE_CATALOG[0]) {
    setSelectedBrand(brand);
  ...
  return (
    <div className="min-h-screen" style={{ background: "#060d1a" }}>
      ...
      {/* ── Compact tool header ── */}
      <div className="px-4 py-2.5" style={{ background: "#06101e", borderBottom: "1px solid #0d1e38" }}>
        <div className="max-w-6xl mx-auto flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg,#2563eb,#4f46e5)" }}>
            <Cpu size={15} className="text-white" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-black text-white text-[13px] leading-none">GSM World</p>
            <p className="text-[9px] font-semibold uppercase tracking-widest mt-0.5" style={{ color: "#415FFF" }}>Direct Unlock Service — v2025</p>
          </div>
          <div className="hidden sm:flex items-center gap-3">
            {[
              { icon: <Shield size={10} />, text: "SSL" },
              { icon: <BadgeCheck size={10} />, text: "15K+ Unlocked" },
              { icon: <Globe size={10} />, text: "Worldwide" },
            ].map(b => (
              <div key={b.text} className="flex items-center gap-1" style={{ color: "#34d399", fontSize: 10, fontWeight: 700 }}>
                {b.icon} {b.text}
              </div>
            ))}
          </div>
          {(selectedBrand || step !== "brand") && (
            <button onClick={copyShareLink}
              ...
              style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", color: "#94b8d8" }}>
      ...
      {/* ── UnlockTool-style brand grid (always visible) ── */}
      <div className="max-w-6xl mx-auto px-2 pt-2">
        <TopBrandBar selectedBrand={selectedBrand} onSelect={handleBrandSelect} />
      </div>

      {/* ── Step progress bar ── */}
      <div className="px-4 py-2.5 sticky top-0 z-20" style={{ background: "rgba(6,13,26,0.97)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(59,130,246,0.1)" }}>
        <div className="max-w-6xl mx-auto flex items-center gap-4">
          <StepBar step={step} />
          {selectedBrand && (
            <div className="hidden sm:flex items-center gap-2 shrink-0 pl-3" style={{ borderLeft: "1px solid rgba(59,130,246,0.2)" }}>
              <div className="w-6 h-6 rounded-md flex items-center justify-center shrink-0"
                style={{ background: BRAND_COLORS[selectedBrand.brand] ?? "#1a2a3a" }}>
                <img src={BRAND_LOGOS[selectedBrand.brand]} alt="" style={{ width: 14, height: 14, objectFit: "contain" }} onError={() => {}} />
              </div>
              <span className="text-[11px] font-bold" style={{ color: "#94b8d8" }}>
                {BRAND_SHORT[selectedBrand.brand] ?? selectedBrand.brand}
                {selectedModel && <> — <span style={{ color: "#34d399" }}>${selectedModel.price}</span></>}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 pb-28">
        <div className="flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-5">
            ...
            {/* ── Step 1: Welcome / Brand prompt ── */}
            {step === "brand" && (
              <div className="space-y-4">
                {/* Instruction panel */}
                <div className="rounded-xl p-5" style={{ background: "#0d1b35", border: "1px solid rgba(59,130,246,0.2)" }}>
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg,#2563eb,#4f46e5)" }}>
                      <Cpu size={22} className="text-white" />
                    </div>
                    <div>
                      <h2 className="font-black text-white text-base leading-tight">Select your device brand above ↑</h2>
                      <p className="text-[12px] mt-1 leading-relaxed" style={{ color: "#6ea4d4" }}>
                        Click any brand in the grid above to start your unlock. {DEVICE_CATALOG.length} brands and {DEVICE_CATALOG.reduce((t, b) => t + b.models.length, 0)}+ models supported.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {[
                    { label: "Brands", value: `${DEVICE_CATALOG.length}`, icon: "🔌", color: "#60a5fa" },
                    { label: "Models", value: `${DEVICE_CATALOG.reduce((t, b) => t + b.models.length, 0)}+`, icon: "📱", color: "#34d399" },
                    { label: "Success Rate", value: "99.2%", icon: "✅", color: "#a78bfa" },
                    { label: "Avg. Time", value: "2–24 hrs", icon: "⚡", color: "#fbbf24" },
                  ].map(s => (
                    <div key={s.label} className="rounded-xl p-3 text-center" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(59,130,246,0.1)" }}>
                      <div className="text-xl mb-1">{s.icon}</div>
                      <p className="font-black text-lg leading-none" style={{ color: s.color }}>{s.value}</p>
                      <p className="text-[10px] mt-1 font-semibold uppercase tracking-wide" style={{ color: "#2d4a6a" }}>{s.label}</p>
                    </div>
                  ))}
                </div>

                {/* How it works */}
                <div className="rounded-xl p-4" style={{ background: "#0d1b35", border: "1px solid rgba(59,130,246,0.15)" }}>
                  <p className="text-[10px] font-black uppercase tracking-widest mb-3" style={{ color: "#2d4a6a" }}>How it works</p>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      { n: "01", title: "Pick Brand", desc: "Click the brand in the grid above" },
                      { n: "02", title: "Select Model", desc: "Choose your exact device model" },
                      { n: "03", title: "Enter IMEI", desc: "Dial *#06# to find your IMEI" },
                      { n: "04", title: "Pay & Receive", desc: "Unlock code sent to your email" },
                    ].map(s => (
                      <div key={s.n} className="rounded-lg p-3" style={{ background: "rgba(37,99,235,0.06)", border: "1px solid rgba(59,130,246,0.1)" }}>
                        <p className="font-black text-xs mb-1" style={{ color: "#415FFF" }}>{s.n}</p>
                        <p className="font-black text-white text-[12px] leading-tight">{s.title}</p>
                        <p className="text-[10px] mt-1 leading-snug" style={{ color: "#2d4a6a" }}>{s.desc}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {step === "model" && selectedBrand && (
              <div className="rounded-2xl overflow-hidden" style={{ background: "#0d1b35", border: "1px solid rgba(59,130,246,0.18)" }}>
                <div className="px-5 py-4 flex items-center gap-3" style={{ borderBottom: "1px solid rgba(59,130,246,0.1)" }}>
                  <button onClick={() => setStep("brand")}
                    ...
                    style={{ border: "1px solid rgba(59,130,246,0.25)", color: "#6ea4d4" }}>
                    <ArrowLeft size={14} />
                  </button>
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)" }}>
                    <BrandLogo brand={selectedBrand} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h2 className="font-black text-white text-base leading-none">{selectedBrand.brand}</h2>
                    <p className="text-[12px] mt-0.5" style={{ color: "#6ea4d4" }}>Select your exact model</p>
                  </div>
                  <span className="text-[10px] font-black px-2.5 py-1 rounded-full shrink-0" style={{ background: "rgba(37,99,235,0.2)", color: "#60a5fa", border: "1px solid rgba(59,130,246,0.3)" }}>{selectedBrand.models.length} models</span>
                </div>
                <div className="p-3 space-y-1.5 max-h-[540px] overflow-y-auto">
                  {selectedBrand.models.map((model) => {
                    ...
                    const tierColors: Record<string, string> = { premium: "rgba(168,85,247,0.15)", high: "rgba(37,99,235,0.15)", mid: "rgba(16,185,129,0.15)", budget: "rgba(100,116,139,0.15)" };
                    const tierTextColors: Record<string, string> = { premium: "#c084fc", high: "#60a5fa", mid: "#34d399", budget: "#94a3b8" };
                    const tierBorders: Record<string, string> = { premium: "rgba(168,85,247,0.3)", high: "rgba(59,130,246,0.3)", mid: "rgba(16,185,129,0.3)", budget: "rgba(100,116,139,0.2)" };
                    ...
                    return (
                      <button key={model.name}
                        ...
                        onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(37,99,235,0.12)"; (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(59,130,246,0.4)"; }}
                        onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(255,255,255,0.03)"; (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(59,130,246,0.1)"; }}>
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>
                          <Smartphone size={14} style={{ color: "#4a6a8a" }} />
                        </div>
                        <p className="text-[13px] font-semibold flex-1 min-w-0 truncate" style={{ color: "#d1e0f0" }}>{model.name}</p>
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="text-[9px] font-black px-2 py-0.5 rounded-full" style={{ background: tierColors[tier], color: tierTextColors[tier], border: `1px solid ${tierBorders[tier]}` }}>{tierLabel[tier]}</span>
                          <span className="text-base font-black" style={{ color: "#34d399" }}>${model.price}</span>
                          <ChevronRight size={12} style={{ color: "#2d4a6a" }} />
                        </div>
                      </button>
                    );
                  })}
              ...
              <div className="space-y-4">
                <div className="rounded-2xl overflow-hidden" style={{ background: "#0d1b35", border: "1px solid rgba(59,130,246,0.18)" }}>
                  <div className="px-5 py-4 flex items-center gap-3" style={{ borderBottom: "1px solid rgba(59,130,246,0.1)" }}>
                    ...
                    <div>
                      <h2 className="font-black text-white text-base leading-none">Device Information</h2>
                      <p className="text-[12px] mt-0.5" style={{ color: "#6ea4d4" }}>{isIPad ? "Enter your Serial Number to continue" : "Enter your IMEI number to continue"}</p>
                    </div>
                  </div>
                  <div className="p-5 space-y-5">
                    <OrderSummaryCard brand={selectedBrand} model={selectedModel} imei={imei || "—"} isIPad={isIPad} />
                    <div className="space-y-1.5">
                      <label className="block text-[11px] font-black uppercase tracking-wider" style={{ color: "#4a6a8a" }}>{isIPad ? "Serial Number" : "IMEI or Serial Number"} <span style={{ color: "#f87171" }}>*</span></label>
                      <div className="relative">
                        <input
                          ...
                          className="w-full rounded-xl px-4 py-3 text-sm font-mono focus:outline-none pr-10"
                        ...
                        {(isIPad ? imei.length >= 8 : imei.length === 15) && (
                          <button onClick={() => { setImeiCopied(true); navigator.clipboard.writeText(imei); setTimeout(() => setImeiCopied(false), 1500); }}
                            className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: "#4a6a8a" }}>
                            {imeiCopied ? <Check size={14} style={{ color: "#34d399" }} /> : <Copy size={14} />}
                          </button>
                        )}
                      </div>
                      <div className="flex items-start gap-2 rounded-xl px-3 py-2.5" style={{ background: "rgba(37,99,235,0.1)", border: "1px solid rgba(59,130,246,0.2)" }}>
                        <Info size={12} className="mt-0.5 shrink-0" style={{ color: "#60a5fa" }} />
                        {isIPad
                           ? <p className="text-[11px]" style={{ color: "#93c5fd" }}>Find your Serial Number in <strong>Settings → General → About</strong>, or on the back of the iPad / original box.</p>
                           : <p className="text-[11px]" style={{ color: "#93c5fd" }}>Dial <strong className="font-mono">*#06#</strong> to get your IMEI, or go to Settings → About Phone → IMEI Information.</p>
                         }
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      <label className="block text-[11px] font-black uppercase tracking-wider" style={{ color: "#4a6a8a" }}>Notes <span style={{ color: "#2d4a6a" }}>(optional)</span></label>
                      <textarea
                        value={notes}
            ...
            {step === "pay" && selectedBrand && selectedModel && (
              <div className="space-y-4">
                {/* ── Inline auth gate — only at payment step ── */}
                {!isAuthenticated && (
                  <div className="rounded-2xl overflow-hidden" style={{ background: "#0d1b35", border: "1px solid rgba(59,130,246,0.3)" }}>
                    <div className="p-6 text-center">
                      <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "linear-gradient(135deg,#2563eb,#4f46e5)" }}>
                        <Lock size={24} className="text-white" />
                      </div>
                      <h3 className="text-lg font-black text-white">Sign In to Pay</h3>
                      <p className="text-[13px] mt-2 mb-5" style={{ color: "#6ea4d4" }}>Create a free account to complete your order for <strong className="text-white">{selectedModel.name}</strong></p>
                      <div className="flex flex-col gap-2.5">
                        <Link href={`/login?redirect=/direct-unlock`}>
                          <button className="w-full py-3 rounded-xl font-bold text-white text-sm transition-all" style={{ background: "linear-gradient(135deg,#2563eb,#4f46e5)" }}>
                            Sign In to Continue
                          </button>
                        </Link>
                        <Link href="/signup">
                          <button className="w-full py-3 rounded-xl font-bold text-sm transition-all" style={{ border: "1px solid rgba(59,130,246,0.3)", color: "#93c5fd" }}>
                            Create Free Account
                          </button>
                        </Link>
                        <p className="text-[11px] mt-1" style={{ color: "#2d4a6a" }}>Free account · No credit card required to register</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-black text-white text-base leading-none">
                      {mpCheckoutId ? "Complete M-Pesa Payment" : npPayment ? "Complete Crypto Payment" : "Choose Payment Method"}
                    </h2>
                    <p className="text-[12px] mt-0.5" style={{ color: "#6ea4d4" }}>Order is created automatically after payment is confirmed</p>
                  </div>
                </div>
          ...
          <div className="hidden md:block">
            <InfoSidebar walletBalance={walletBalance} selectedModel={selectedModel} isAuthenticated={isAuthenticated} />
          </div>
