 ...
 * whether the scrollable element is scrolled past the top.
 *
 ...
 */
 ...
 // Track every scrollable admin panel, especially the inner live-chat
 // message list. The page scrolls inside overflow-y-auto, so the WebView's
 // own scroll position remains zero while the user reads earlier messages.
 // Without this bridge SwipeRefreshLayout mistakes that gesture for a
 // pull-to-refresh and reloads the entire admin app.
 view.evaluateJavascript(
 "(function(){" +
 " if(typeof AdminScrollBridge==='undefined') return;" +
 " var tracked=[];" +
 " function refresh(){" +
 "  var scrolled=window.scrollY>0 || tracked.some(function(el){return el.scrollTop>0;});" +
 "  AdminScrollBridge.setScrolled(scrolled);" +
 " }" +
 " function attach(){" +
 "  tracked=Array.prototype.slice.call(document.querySelectorAll('main,.overflow-y-auto,.overflow-auto'));" +
 "  tracked=tracked.filter(function(el){return el===document.body || el.scrollHeight>el.clientHeight;});" +
 "  tracked.forEach(function(el){el.addEventListener('scroll',refresh,{passive:true});});" +
 "  refresh();" +
 " }" +
 " attach();" +
