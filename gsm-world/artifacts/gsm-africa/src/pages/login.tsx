...
import { Eye, EyeOff, ArrowRight, Mail, Lock, Hash, Smartphone, Loader2, Fingerprint } from "lucide-react";
...
export function LoginPage() {
  ...
  async function handleFingerprintLogin() {
    ...
    try {
      ...
      const challengeData = await challengeRes.json() as { _userId?: number; error?: string; [k: string]: unknown };
      if (!challengeRes.ok) {
        toast({ title: challengeData.error ?? "No fingerprint registered for this account.", variant: "destructive" });
        return;
      }
      const { _userId, ...options } = challengeData;
      const assertion = await startAuthentication({ optionsJSON: options as Parameters<typeof startAuthentication>[0]["optionsJSON"] });
      const authRes = await fetch(`${base}/api/auth/webauthn/auth`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...assertion, _userId }),
      });
      const authData = await authRes.json() as { token?: string; user?: { id: number; email: string; name: string | null }; error?: string };
      if (!authRes.ok) {
        toast({ title: authData.error ?? "Fingerprint login failed", variant: "destructive" });
        return;
      }
      login(authData.token!, authData.user!);
      toast({ title: "Welcome back!", description: `Signed in as ${authData.user!.email}` });
      navigate(returnTo);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      if (msg.includes("NotAllowedError") || msg.toLowerCase().includes("cancel")) {
        toast({ title: "Fingerprint sign-in cancelled", variant: "destructive" });
      } else {
        toast({ title: "Fingerprint login failed. Please try again.", variant: "destructive" });
      }
    } finally {
      setFpLoading(false);
    }
          ...
          {googlePolling ? (
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                disabled={fpLoading}
                className="w-full flex items-center gap-3 rounded-2xl transition-all active:scale-[0.98]"
                style={{ background: "rgba(99,102,241,0.10)", border: "1px solid rgba(99,102,241,0.22)", padding: "12px 16px" }}
                onMouseEnter={(e) => { if (!fpLoading) e.currentTarget.style.background = "rgba(99,102,241,0.18)"; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(99,102,241,0.10)"; }}
              >
                {fpLoading ? (
                  <Loader2 size={18} className="animate-spin shrink-0" style={{ color: "#a5b4fc" }} />
                ) : (
                  <Fingerprint size={18} className="shrink-0" style={{ color: "#a5b4fc" }} />
                )}
                <span className="flex-1 text-center text-sm font-semibold" style={{ color: "#c7d2fe" }}>
                  {fpLoading ? "Verifying fingerprint…" : "Sign in with Fingerprint"}
                </span>
              </button>
            </div>
          )}
