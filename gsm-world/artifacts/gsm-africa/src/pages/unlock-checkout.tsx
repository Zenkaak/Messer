...
const PROCESSING_DURATION_MS = 5 * 60 * 1000;
...
function Processing({ device, onContinue, onBack }: { device: Device; onContinue: () => void; onBack: () => void }) {
  ...
  const [remainingMs, setRemainingMs] = useState(PROCESSING_DURATION_MS);
  useEffect(() => {
    const startedAt = Date.now();
    const update = () => {
      const elapsed = Date.now() - startedAt;
      const remaining = Math.max(0, PROCESSING_DURATION_MS - elapsed);
  ...
  useEffect(() => {
    ...
    const warnBeforeLeaving = (event: BeforeUnloadEvent) => {
      ...
      event.returnValue = "Your unlock check is still running. Keep this page open.";
    };
  ...
  const progress = ready ? 100 : Math.min(99, Math.round(((PROCESSING_DURATION_MS - remainingMs) / PROCESSING_DURATION_MS) * 100));
  return <div className="fade-swap">
    ...
    <div className="relative overflow-hidden rounded-2xl border border-primary/20 bg-[#e5f0ec] p-6 dark:bg-secondary"><div className="scan-line absolute left-0 right-0 h-20 bg-gradient-to-b from-transparent via-primary/10 to-transparent" /><div className="relative flex items-center gap-4"><span className={`grid h-12 w-12 shrink-0 place-items-center rounded-full ${ready ? "bg-primary text-primary-foreground" : "bg-card text-primary"}`}>{ready ? <Check size={23} /> : <RefreshCw size={21} className="animate-spin" />}</span><span><span className="block text-sm font-bold">{ready ? "Eligibility confirmed" : "Verifying your device"}</span><span className="mt-1 block text-xs text-muted-foreground">{device.brand} {device.model} · {device.series}</span></span><span className="mono ml-auto shrink-0 text-sm font-medium text-primary">{ready ? "Done" : `${minutes}:${seconds}`}</span></div><div className="relative mt-6 h-2 overflow-hidden rounded-full bg-card/80"><div className="h-full rounded-full bg-primary transition-[width] duration-1000 ease-linear" style={{ width: `${progress}%` }} /></div><div className="relative mt-2 flex justify-between text-[10px] font-semibold uppercase tracking-[.12em] text-muted-foreground"><span>{ready ? "Complete" : "Secure check in progress"}</span><span>{ready ? "Ready for payment" : "Estimated 5–8 minutes"}</span></div></div>
    <div className="mt-5 rounded-2xl border border-[#e9cda5] bg-[#fff5e5] p-4 text-sm leading-6 text-[#704a22] dark:bg-[#3d3020] dark:text-[#f0c995]"><div className="flex gap-3"><Clock3 className="mt-0.5 shrink-0" size={18} /><span><strong>Do not close or refresh this page.</strong> The secure device check runs for about 5–8 minutes. Once it is complete, you’ll review the selected device price and choose your payment method.</span></div></div>
...
function Payment({ device, email, onBack, onPay, submitting, error }: { device: Device; email: string; onBack: () => void; onPay: (method: "card" | "mobile") => void; submitting: boolean; error: string | null }) {
  ...
  return <div className="fade-swap">
    ...
    <div className="rounded-xl border border-border bg-card p-4 text-xs leading-5 text-muted-foreground"><ShieldCheck className="mb-2 text-primary" size={18} /><strong className="text-foreground">Payment is handled securely.</strong> We do not store card details. This starts a payment request; your order is not marked paid until payment is confirmed.</div>
    <div className="mt-8 flex flex-col-reverse gap-3 border-t border-border pt-6 sm:flex-row sm:items-center sm:justify-between"><button type="button" onClick={onBack} disabled={submitting} className="flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-bold text-muted-foreground hover:bg-secondary hover:text-foreground disabled:opacity-50"><ArrowLeft size={16} /> Back</button><button type="button" onClick={() => onPay(method)} disabled={submitting} className="primary-btn flex min-h-11 items-center justify-center gap-2 rounded-xl bg-accent px-5 text-sm font-bold text-accent-foreground disabled:cursor-wait disabled:opacity-60">{submitting ? "Submitting request…" : `Request payment ${formatPrice(device.price)}`} {!submitting && <ArrowRight size={16} />}</button></div>
  </div>;
}

function Pending({ device, email, identifier, reference: providedReference, onReset }: { device: Device; email: string; identifier: string; reference?: string; onReset: () => void }) {
  const reference = providedReference || `GSM-${identifier.replace(/\W/g, "").slice(-6).toUpperCase() || "WORLD"}`;
...
export function UnlockCheckoutPage() {
  ...
  const reset = () => { setStage("device"); setDevice(null); setIdentifier(""); setEmail(""); setOrderReference(undefined); setPaymentError(null); setLocation("/"); };
  const submitPaymentRequest = async (method: "card" | "mobile") => {
    if (!device) return;
    ...
    try {
      const response = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          sessionId: `direct-unlock-${Date.now()}`,
          customerEmail: email,
          paymentMethod: method,
          paymentStatus: "pending_payment_confirmation",
          total: device.price.toFixed(2),
          currency: "USD",
          ...
          items: [{
            productId: 0,
            productName: `${device.brand} Direct Unlock — ${device.model}`,
