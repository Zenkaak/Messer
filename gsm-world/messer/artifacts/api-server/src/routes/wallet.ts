...
router.post("/wallet/add-fund/mpesa/callback", async (req, res) => {
  try {
    ...
    if (pending && resultCode === 0) {
      ...
      const afterRows = await db.select({ walletBalance: usersTable.walletBalance }).from(usersTable).where(eq(usersTable.id, userId)).limit(1);
      const newBalance = parseFloat(afterRows[0]?.walletBalance ?? "0");
      logger.info({ userId, amountUsd }, "Wallet top-up credited via M-Pesa callback");
      sendEmail({ to: email, ...walletCreditEmail({ name: null, email, amount: amountUsd, newBalance, method: "M-Pesa" }) }).catch((e) => logger.error({ e }, "Email send failed"));
    } else if (pending) {
      walletTopUpPending.delete(checkoutRequestId);
