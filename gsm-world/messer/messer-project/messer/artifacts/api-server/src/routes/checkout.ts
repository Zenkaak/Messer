...
function addPaymentNotification(n: Omit<PaymentNotification, "id" | "timestamp" | "read">) {
  paymentNotifications.unshift({
    ...n,
    id: `notif-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    timestamp: Date.now(),
    read: false,
  });
  if (paymentNotifications.length > 50) paymentNotifications.pop();
  logger.info({ orderId: n.orderId, paymentMethod: n.paymentMethod }, "Payment confirmed — admin notification created");
}
...
router.post("/checkout", async (req, res) => {
  try {
    ...
    if (paymentMethod === "wallet") {
      ...
      addPaymentNotification({ orderId: order.id, amount: String(total), paymentMethod: "wallet", customerEmail: customerEmail.toLowerCase(), customerName: customerName ?? null });
      responseStatus = "paid";
...
// ─── Custom denomination order (no product in catalog required) ───────────────
const CreateCustomOrderBody = z.object({
...
router.post("/checkout/custom", async (req, res) => {
  try {
    ...
    const { brand, region, denomination, customerEmail, customerName, customerPhone, paymentMethod, payCurrency, notes, sessionId: bodySessionId } = parsed.data;

    const sessionId = bodySessionId ?? resolveSessionId(req);
    const total = denomination;
    const itemName = `${brand} Gift Card — ${region} $${denomination.toFixed(2)} (Custom Denomination)`;
    const orderNotes = notes
      ? `${notes}\nBrand: ${brand} | Region: ${region} | Amount: $${denomination}`
      : `Custom gift card — Brand: ${brand} | Region: ${region} | Amount: $${denomination}`;
    ...
    const [order] = await db.insert(ordersTable).values({
      ...
      currency: "USD",
      notes: orderNotes,
      orderType: "custom_gift_card",
      userId: loggedInUserId,
    }).returning();

    // productId 0 = sentinel for custom/no-catalog orders
    await db.insert(orderItemsTable).values({
      orderId: order.id,
      productId: 0,
      productName: itemName,
      price: String(denomination),
      quantity: 1,
    });
    ...
    if (paymentMethod === "wallet") {
      ...
      const userRows = await db.select({ walletBalance: usersTable.walletBalance }).from(usersTable).where(eq(usersTable.id, loggedInUserId)).limit(1);
      const balance = parseFloat(userRows[0]?.walletBalance ?? "0");
      if (balance < total) {
        res.status(400).json({ error: `Insufficient wallet balance. You have $${balance.toFixed(2)} but need $${total.toFixed(2)}.` });
        return;
      ...
      sendEmail({ to: customerEmail, ...paymentConfirmedEmail({ orderId: order.id, customerName: customerName ?? null, amount: String(total), paymentMethod: "wallet" }) }).catch(() => {});
      responseStatus = "paid";
      ...
      const amountKes = Math.ceil(total * USD_TO_KES);
      try {
        const stkRes = await initiateSTKPush({ phone: customerPhone, amount: amountKes, orderId: order.id, description: `Order #${order.id} — ${brand} ${region}` });
        ...
        logger.error({ err }, "Custom order STK Push failed");
        await db.update(ordersTable).set({ paymentStatus: "failed" }).where(eq(ordersTable.id, order.id));
      ...
      try {
        const payment = await createPayment({ priceAmount: total, priceCurrency: "usd", payCurrency: currency, orderId: `custom-${order.id}`, orderDescription: `${brand} ${region} Gift Card $${denomination} #${order.id}` });
        ...
        nowpaymentsData = { paymentId: payment.payment_id, payAddress: payment.pay_address, payAmount: payment.pay_amount, payCurrency: payment.pay_currency, expiresAt: payment.expiration_estimate_date };
      } catch (payErr) {
        const msg = payErr instanceof Error ? payErr.message : "NOWPayments error";
        logger.error({ payErr }, "Custom order NOWPayments failed");
        await db.update(ordersTable).set({ paymentStatus: "failed" }).where(eq(ordersTable.id, order.id));
      ...
      usdtData = { walletAddress, network: network ?? "TRC20", amountUsdt: parseFloat(total.toFixed(2)), memo: `ORDER-${order.id}` };

    } else if (paymentMethod === "binance_pay") {
      const binanceId = await getBinancePayId();
      await db.update(ordersTable).set({ paymentStatus: "pending_payment_confirmation" }).where(eq(ordersTable.id, order.id));
      await db.insert(paymentTransactionsTable).values({ orderId: order.id, provider: "binance_pay", providerReference: null, amount: String(total), currency: "USD", status: "pending", rawResponse: { binanceId, reference: `ORDER-${order.id}` } as Record<string, unknown> });
      customData = { method: "binance_pay", binanceId, amount: total, reference: `ORDER-${order.id}` };
      responseStatus = "pending_payment_confirmation";
    }

    // Confirmation emails
    const itemsForEmail = [{ productName: itemName, quantity: 1, price: String(denomination) }];
    if (paymentMethod !== "binance_pay") {
      sendEmail({ to: customerEmail, ...orderSubmittedEmail({ orderId: order.id, customerName: customerName ?? null, items: itemsForEmail, total: String(total), paymentMethod }) }).catch(() => {});
    }
    // Admin alert
    getSmtpConfig().then(cfg => {
      if (cfg.emailFrom) {
        sendEmail({ to: cfg.emailFrom, ...adminNewOrderAlertEmail({ orderId: order.id, orderType: "Custom Gift Card", customerEmail, customerName: customerName ?? null, items: itemName, total: String(total), paymentMethod }) }).catch(() => {});
      }
    }).catch(() => {});

    res.json({ orderId: order.id, paymentMethod, status: responseStatus, total, currency: "USD", mpesa: mpesaData, usdt: usdtData, custom: customData, nowpayments: nowpaymentsData });
  } catch (err) {
    logger.error({ err }, "Custom order checkout failed");
    res.status(500).json({ error: "Internal server error" });
  }
});
  ...
  try {
    ...
    try {
      ...
      if (paidStatuses.includes(status.payment_status)) {
        ...
        if (orderRows[0]) {
          addPaymentNotification({ orderId, amount: orderRows[0].total, paymentMethod: "nowpayments", customerEmail: orderRows[0].customerEmail, customerName: orderRows[0].customerName });
        }
  ...
  try {
    ...
    try {
      ...
      if (resultCode === "0") {
        ...
        if (orderRows[0]) {
          addPaymentNotification({ orderId, amount: orderRows[0].total, paymentMethod: "mpesa", customerEmail: orderRows[0].customerEmail, customerName: orderRows[0].customerName });
        }
        res.json({ paymentStatus: "paid", message: "Payment confirmed" });
    ...
    if (txRows.length) {
      ...
      if (status === "paid") {
        ...
        if (orderRows[0]) {
          ...
          addPaymentNotification({ orderId, amount: total, paymentMethod: "mpesa", customerEmail, customerName });
...
router.get("/admin/notifications", (_req, res) => {
  const unread = paymentNotifications.filter(n => !n.read).length;
  res.json({ notifications: paymentNotifications.slice(0, 30), unread });
});

router.post("/admin/notifications/mark-read", (_req, res) => {
  paymentNotifications.forEach(n => { n.read = true; });
  res.json({ ok: true });
});
