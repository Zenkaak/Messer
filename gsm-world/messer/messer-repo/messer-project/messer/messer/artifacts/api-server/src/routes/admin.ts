...
router.post("/admin/test-email", async (req, res) => {
  try {
    ...
    const result = await sendEmail({
      to,
      subject: "GSM World - Email Configuration Test",
      text: "This is a test email from your GSM World admin panel. If you received this, your email configuration is working correctly.",
      html: `<div style="font-family:Helvetica,Arial,sans-serif;padding:24px;max-width:520px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;">
        <h2 style="color:#0f172a;margin:0 0 12px;font-size:18px;">Email Configuration Test</h2>
        <p style="color:#475569;margin:0 0 16px;">This is a test email from your <strong>GSM World</strong> admin panel.</p>
        <p style="color:#059669;font-weight:700;margin:0 0 16px;">Your email configuration is working correctly.</p>
        <hr style="border:none;border-top:1px solid #e2e8f0;margin:20px 0;">
        <p style="color:#94a3b8;font-size:12px;margin:0;">Sent from GSM World Admin Panel</p>
      </div>`,
    });
    req.log.info({ to, result }, "Admin test email sent");
    res.json(result);
  } catch (err) {
