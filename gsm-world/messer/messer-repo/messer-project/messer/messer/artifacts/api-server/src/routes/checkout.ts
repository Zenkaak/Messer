...
router.post("/checkout", async (req, res) => {
  try {
    ...
    // Insert in-app notification for logged-in users
    if (loggedInUserId) {
      db.insert(notificationsTable).values({
        userId: loggedInUserId,
        title: "Order Received",
        message: `Your order #${order.id} has been received and is being reviewed. Estimated processing: 10–30 minutes.`,
        type: "success",
        orderId: order.id,
      }).catch((err: unknown) => logger.error({ err }, "Failed to insert order notification"));
    }
