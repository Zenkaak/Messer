...
function OverviewPanel({ pwd, onNavigate }: { pwd: string; onNavigate: (tab: Tab) => void }) {
  ...
  const pending = (stats?.orders.total ?? 0) - (stats?.paidOrders.count ?? 0);
  const convRate = stats?.orders.total ? Math.round((stats.paidOrders.count / stats.orders.total) * 100) : 0;
  const avgOrder = stats?.paidOrders.count ? (stats.paidOrders.revenue / stats.paidOrders.count).toFixed(0) : "0";
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: "Total Orders",   value: stats?.orders.total ?? 0,  sub: `${pending} pending · ${stats?.paidOrders.count ?? 0} paid`, icon: <ShoppingBag size={15} />, from: "#3b82f6", to: "#1d4ed8" },
              { label: "Conversion",     value: `${convRate}%`,             sub: "paid vs total orders",   icon: <TrendingUp size={15} />,   from: "#10b981", to: "#065f46" },
