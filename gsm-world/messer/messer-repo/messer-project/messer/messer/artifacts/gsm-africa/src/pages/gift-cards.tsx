...
function BrandLogo({ brand, emoji, size = "md" }: { brand: string; emoji: string; size?: "sm" | "md" | "lg" }) {
  ...
  if (currentUrl) {
    return (
      <img
        src={currentUrl}
        className={`${dim} object-contain rounded-xl bg-white p-1 shadow-sm`}
...
export function GiftCardsPage() {
  ...
  async function handlePlaceOrder() {
    ...
    const priceUsd = isUsd ? selectedDenom : (customUsdPrice > 0 ? customUsdPrice : undefined);
    const needsUsdPrice = !isUsd && ["wallet", "mpesa", "nowpayments"].includes(payMethod);

    if (needsUsdPrice && (!priceUsd || priceUsd <= 0)) {
      toast({ variant: "destructive", title: "USD price required", description: "Enter the USD equivalent price for this card." });
      return;
    }
    if (payMethod === "wallet" && walletBalance < (priceUsd ?? 0)) {
          ...
          <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            {gcStep.type === "done" ? (
              ...
              <div className="p-5 space-y-4">
                  ...
                  <div className="space-y-2.5">
                    ...
                    <button type="button" onClick={() => setPayMethod("mpesa")}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">STK Push · Kenya{selectedCard.currency === "USD" ? ` · KES ${Math.ceil(selectedDenom * 130).toLocaleString()}` : ""}</p>
                      </div>
                    ...
                    <button type="button" onClick={() => setPayMethod("wallet")}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs mt-0.5">
                          <span className="text-green-600 font-semibold">${walletBalance.toFixed(2)} available</span>
                    ...
                    <button type="button" onClick={() => setPayMethod("nowpayments")}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">BTC · ETH · USDT · 100+ coins · min $13</p>
                      </div>
                ...
                {/* USD equivalent price — required for non-USD cards with automatic payment methods */}
                {selectedCard.currency !== "USD" && payMethod && ["wallet", "mpesa", "nowpayments"].includes(payMethod) && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3.5">
                    <label className="text-xs font-bold text-amber-800 uppercase tracking-wider block mb-1.5">
                      USD Equivalent Price (required)
                    </label>
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) => setCustomUsdPrice(parseFloat(e.target.value) || 0)}
                        className="flex-1 px-3 py-2 border-2 border-amber-300 rounded-xl text-sm font-bold focus:outline-none focus:border-amber-500 bg-white"
                      />
                      <span className="text-xs text-amber-700 font-semibold">USD</span>
                    </div>
                    <p className="text-[11px] text-amber-700 mt-1.5">
                      Enter the USD price for this {selectedCard.currency} {selectedCard.denomination ?? selectedDenom} card. Contact support if unsure.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={handlePlaceOrder}
                    disabled={
                      loading || !payMethod ||
                      (payMethod === "wallet" && selectedCard.currency === "USD" && walletBalance < selectedDenom) ||
                      (payMethod === "wallet" && selectedCard.currency !== "USD" && walletBalance < (customUsdPrice || 0)) ||
                      (selectedCard.currency !== "USD" && !!payMethod && ["wallet","mpesa","nowpayments"].includes(payMethod) && !customUsdPrice)
                    }
                ...
                {filtered.map((card) => (
                  ...
                  >
                    <div
                      ...
                      style={{ background: `linear-gradient(135deg, ${card.bg} 0%, ${card.accent} 100%)` }}
                    >
                      <BrandLogo brand={card.brand} emoji={card.emoji} size="lg" />
                    </div>
                    <div className="px-3 py-2.5">
                      <p className="font-black text-gray-800 text-[12px] leading-tight line-clamp-1">{card.brand}</p>
                      <p className="text-[10px] text-gray-400 mt-0.5 line-clamp-1">{card.region}</p>
                      <div className="flex items-center justify-between mt-1.5">
                        <p className="text-[11px] font-semibold text-gray-500">
                          from {card.symbol}{Math.min(...card.denominations).toLocaleString()}
                        </p>
