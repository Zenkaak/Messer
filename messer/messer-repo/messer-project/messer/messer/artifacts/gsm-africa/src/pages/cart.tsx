  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="flex flex-col min-h-[80vh] bg-white">
        <div style={{ background: "linear-gradient(160deg,#1a2332 0%,#1e3a5f 100%)" }} className="flex flex-col items-center justify-center pt-14 pb-12 px-6 text-center">
          <div className="w-20 h-20 rounded-3xl bg-white/10 border border-white/20 flex items-center justify-center mb-4 shadow-lg shadow-slate-950/20">
            <ShoppingBag size={36} className="text-white/60" />
          </div>
          <h2 className="text-2xl font-black text-white mb-1">Cart is Empty</h2>
          <p className="text-blue-300/70 text-sm max-w-[220px]">Add a few products to start your checkout.</p>
        </div>
        <div className="px-5 pt-6 space-y-3">
          <Link href="/products">
            <button className="w-full py-3.5 bg-[#1a2332] text-white font-black text-base rounded-2xl shadow-lg flex items-center justify-center gap-2">
              Browse Products <ArrowRight size={18} />
            </button>
          </Link>
          <Link href="/categories">
            <button className="w-full py-3.5 border-2 border-gray-200 text-gray-700 font-bold text-base rounded-2xl">
              View Categories
            </button>
          </Link>
        </div>
